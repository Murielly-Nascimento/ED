...
TipoGrafo *CriarGrafo(int NVertices)
{
    ...
    //Alocamos um vetor
    Grafo->Mat = (ATORES **)malloc(NVertices * sizeof(ATORES *));
    //Se a alocação deu errado
    if (Grafo->Mat == NULL)
    {
        free(Grafo);
    ...
    for (i = 0; i < NVertices; i++)
    {
        ...
        Grafo->Mat[i] = (ATORES *)calloc(NVertices, sizeof(ATORES));
        //Se a alocação deu errado
        if (Grafo->Mat[i] == NULL)
        {
...
int insereVertice(TipoGrafo *G, int posicao, char *ator)
{
    if (G == NULL)
        return -1;

    if (posicao > G->NumVertices || posicao < 0)
        return -1;

    if (G->Mat[posicao][0].valor != 0)
        return -1;

    strcpy(G->Mat[posicao][0].ator, ator);
}
...
*/
int inserirAresta(TipoGrafo *G, int v1, int v2, char *filme)
{
    if (G == NULL)
        //Grafo não existe
        return -1;
    ...
    */
    ...
    if (G->Mat[v1][v2].valor == 0)
    {
        G->Mat[v1][v2].valor = 1;
        strcpy(G->Mat[v1][v2].filme, filme);
        strcpy(G->Mat[v1][v2].ator, G->Mat[v2][0].ator);
        G->NumArestas++;
    }
    return 1;
}
...
void exibirMatriz(TipoGrafo *G)
{
    ...
    {
        printf("%s:", G->Mat[v][0].ator);
        for (w = 1; w < G->NumVertices; w++)
        {
            printf(" %s", G->Mat[v][w].ator);
            printf(" - %s", G->Mat[v][w].filme);
            printf("        ");
        }
        printf("\n");
    }
...
void exibirGrafo(TipoGrafo *G)
{
    ...
    for (v = 0; v < G->NumVertices; ++v)
    {
        ...
        for (w = 1; w < G->NumVertices; ++w)
        {
            if (G->Mat[v][w].valor == 1)
            {
                printf("Está ligada a %s", G->Mat[v][w].ator);
                printf(",no filme %s", G->Mat[v][w].filme);
                printf("       ");
            }
        }
        printf("\n");
...
int adjacencias(TipoGrafo *Grafo, int v)
{
    ...
    */
    ...
    {
        if (Grafo->Mat[v][i].valor == 1)
            printf(" %s,", Grafo->Mat[v][i].ator);
    }
    printf("\n");
...
int main(void)
{
    ...
    insereVertice(g, 0, "Madonna");
    insereVertice(g, 1, "Marshall Penny");
    insereVertice(g, 2, "Hanks Tom");

    inserirAresta(g, 0, 1, "A League");
    inserirAresta(g, 0, 2, "A League");
    exibirMatriz(g);
    exibirGrafo(g);
    adjacencias(g, 0);
    retirarAresta(g, 0, 1);
