...
int list_erase_zeros(List *list){
  ...
  DLNode *aux = list->begin;
  int contador = 1;
  while((aux!=NULL) && (contador <= list->size)){
    if((aux->data.n1 == 0) && (aux->data.n2 == 0) && (aux->data.n3 == 0)){
      list_erase(list,contador);
      aux = list->begin;// check:<<<erro: acaba sempre voltando para o início?>>>>
      contador--;
    }
    else{
      aux = aux->next;
...
int list_splice(List *dest, List *source, int pos){
  ...
  while((aux!=NULL) && pos<=source->size){
    list_insert(dest,pos,aux->data);
    aux = aux->next;
    pos++;
  }
  return SUCCESS;
